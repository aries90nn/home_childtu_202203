...
<c:set var="b_sbjclr" value="${board.b_sbjclr}" />
<c:if test='${empty board.b_sbjclr}'>
	<c:set var="b_sbjclr" value="red_day" />
</c:if>
...
<script type="text/javascript">//<![CDATA[
	...
	function w_chk(){
		...
		<c:if test="${config.a_show eq 'Y'}">
		var sDate = $( "input[name='b_show_sdate']" ).val(); //2017-12-10
		var sDateArr = sDate.split('-');
		var eDate = $( "input[name='b_show_edate']" ).val(); //2017-12-09
		var eDateArr = eDate.split('-');

		var sTime = $( "input[name='b_show_sdate_time']" ).val(); //00:00
		var sTimeArr = sTime.split(':');

		var eTime = $( "input[name='b_show_edate_time']" ).val(); //23:59
		var eTimeArr = eTime.split(':');

		var sDateCompare = new Date(sDateArr[0], parseInt(sDateArr[1])-1, sDateArr[2], sTimeArr[0], sTimeArr[1]);
		var eDateCompare = new Date(eDateArr[0], parseInt(eDateArr[1])-1, eDateArr[2], eTimeArr[0], eTimeArr[1]);
		</c:if>
		...
		else if (CheckSpaces(document.getElementById('frm').b_sdate, '행사일정 시작일')) {return false;}
		else if (CheckSpaces(document.getElementById('frm').b_edate, '행사일정 종료일')) {return false;}
			
		var startDate = $( "input[name='b_sdate']" ).val(); //2017-12-10
		var startDateArr = startDate.split('-');
		var endDate = $( "input[name='b_edate']" ).val(); //2017-12-09
...
<div id="board" style="width:${config.a_width};">
	...
	<div class="table_bwrite">	
			...
			<thead>
				<tr>
					<th scope="col" colspan="2" class="th_end">글쓰기</th>
				</tr>
			</thead>
			<tbody>
				<tr>
				...
				<!-- 일정 -->
				<tr>
					<th scope="row"><img alt="*(필수항목)" src="/nanum/site/board/nninc_calendar/img/ic_vcheck.gif"/> 행사일정</th>
					<td>
						<jsp:include page="../inc_write_b_date.jsp" />
					</td>
				</tr>

				<tr>
					<th scope="row"><label for="b_sbjclr_1"><img alt="*(필수항목)" src="/nanum/site/board/nninc_calendar/img/ic_vcheck.gif"/> 일정표시색상</label></th>
					<td>
						<input type="radio" id="b_sbjclr_1" name="b_sbjclr" value="red_day" ${b_sbjclr eq 'red_day' ? 'checked' : '' }><label for="b_sbjclr_1" style="color:#f44d4d;">■</label>
						<input type="radio" id="b_sbjclr_2" name="b_sbjclr" value="blue_day" ${b_sbjclr eq 'blue_day' ? 'checked' : '' }><label for="b_sbjclr_2" style="color:#2894e0;">■</label>
						<input type="radio" id="b_sbjclr_3" name="b_sbjclr" value="orange_day" ${b_sbjclr eq 'orange_day' ? 'checked' : '' }><label for="b_sbjclr_3" style="color:#ff7302;">■</label>
						<input type="radio" id="b_sbjclr_4" name="b_sbjclr" value="green_day" ${b_sbjclr eq 'green_day' ? 'checked' : '' }><label for="b_sbjclr_4" style="color:#64aa20;">■</label>
						<input type="radio" id="b_sbjclr_5" name="b_sbjclr" value="yellow_day" ${b_sbjclr eq 'yellow_day' ? 'checked' : '' }><label for="b_sbjclr_5" style="color:#ffd200;">■</label>
						<input type="radio" id="b_sbjclr_6" name="b_sbjclr" value="purple_day" ${b_sbjclr eq 'purple_day' ? 'checked' : '' }><label for="b_sbjclr_6" style="color:#9f29dc;">■</label>
					</td>
				</tr>

				<!-- 사용자추가필드 -->
				<jsp:include page="../inc_write_b_temp.jsp" />
