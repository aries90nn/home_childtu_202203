...
<c:choose>
	<c:when test="${board.a_site_dir eq 'main' }"><c:set var="lib_title" value="<span class='n_lib_name main'>통합</span>" /></c:when>
	<c:when test="${board.a_site_dir eq 'student' }"><c:set var="lib_title" value="<span class='n_lib_name student'>독립</span>" /></c:when>
	<c:when test="${board.a_site_dir eq 'geumho' }"><c:set var="lib_title" value="<span class='n_lib_name geumho'>금호</span>" /></c:when>
	<c:when test="${board.a_site_dir eq 'gecs' }"><c:set var="lib_title" value="<span class='n_lib_name gecs'>교육</span>" /></c:when>
	<c:when test="${board.a_site_dir eq 'jungang' }"><c:set var="lib_title" value="<span class='n_lib_name jungang'>중앙</span>" /></c:when>
	<c:when test="${board.a_site_dir eq 'songjung' }"><c:set var="lib_title" value="<span class='n_lib_name songjeong'>송정</span>" /></c:when>
	<c:when test="${board.a_site_dir eq 'seokbong' }"><c:set var="lib_title" value="<span class='n_lib_name seokbong'>석봉</span>" /></c:when>
	<c:otherwise><c:set var="lib_title" value="[${board.a_site_dir }]" /></c:otherwise>
</c:choose>
...
<div id="board" style="width:${config.a_width};">
	...
	<div class="table_bview">
		...
		<thead>
			<tr>
				<th scope="col">${lib_title }<span class='category_color'>${b_cate_str[board.b_cate]}</span> ${board.b_subject}</th>
			</tr>
		</thead>
		<tbody>
			<tr>
					...
					<dl class="info">
						...
						<dd>
							...
							<c:if test="${file_name ne ''}">
								<img src="/nanum/site/board/${config.a_level}/img/file/${file_icon}" alt="" /><a href="/board/down.do?a_num=${board.a_num}&b_num=${board.b_num}&f_num=${t}" target="_blank">${file_name}</a>
						...
						<c:forEach var="t" begin="1" end="5" step="1">
							...
							<c:if test="${file_name ne ''}">
								...
								<c:if test="${ext eq 'gif' or ext eq 'jpg' or ext eq 'png'}">
									<c:set var="tot_file" value="/board/get_img.do?a_num=${board.a_num}&b_num=${board.b_num}&f_num=${t}&type=img"/>
	...
	<div class="board_button">
		...
		<div class="fr pt10">
			...
			<c:if test="${is_write eq 'Y'}">
				...
				<c:if test="${board.a_num eq config.a_num }">
