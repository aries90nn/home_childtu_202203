...
<div id="board" style="width:${config.a_width};">
	...
	<c:forEach items="${boardList}" var="board" varStatus="no">
		...
		<c:if test="${is_ad_cms ne 'Y' }">
			...
			<%--광주 추가 공개게시물도 이름일부 블라인드 --%>
			<c:if test="${config.a_type eq 'Y' and board.b_open eq 'Y'}">
	...
	<tr>
		...
		<td>${board.b_temp10 eq 'N' ? '감추기' : recordcount - ((v_page-1) * pagesize + no.index)}</td>
	...
	<div class="board_button">
		...
		<div class="fl">
			<ul>
				...
				<c:if test="${param.a_num eq  '69394427'}">
				<br/>
				<li class="pt"> 
					<select id="tot_level_chk" name="tot_level_chk" title="선택한 게시글 상태 선택" style="background: #f8f8f8;font-size: 14px;margin: 0;width: 103px;float: left;margin-right: 3px;padding: 5px 12px;border: 1px solid #ccc;height: 38px;">
						<option value="N">감추기</option>
						<option value="Y">보이기</option>
					</select>
					
					<a href="javascript:tot_levelchage('prepage=${nowPageEncode }');" class="board_lbtn" style="float:left;"><span>선택 게시글변경</span></a>
				</li>
				</c:if>
			</ul>
		</div>
