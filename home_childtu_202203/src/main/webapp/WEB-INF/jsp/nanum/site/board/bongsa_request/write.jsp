...
<fmt:parseDate value="${board.b_sdate}" pattern="yyyy-MM-dd" var="thisdate" />
<fmt:formatDate value="${thisdate}" pattern="E" var="thisweek"/>
...
<c:if test="${empty board.b_num }">
	<c:if test="${!empty sessionScope.ss_user_birthday }">
		<c:set target="${board }" property="b_temp6" value="${fn:replace(sessionScope.ss_user_birthday, '/', '-') }" />
	</c:if>
</c:if>
...
<script type="text/javascript">//<![CDATA[
	...
	function w_chk(){
		...
		//봉사활동시간
		eForm.b_temp5.value = eForm.b_temp5_stime_h.value + ":" + eForm.b_temp5_stime_m.value;
		eForm.b_temp5.value += "~" + eForm.b_temp5_etime_h.value + ":" + eForm.b_temp5_etime_m.value;
		...
		else if (CheckSpaces(eForm.b_temp7, "신청자 연락처")) {return false;}
		/*
		...
		*/
		...
		else if (CheckSpaces(eForm.b_temp5_stime_h, "봉사활동시작시간")) {return false;}
		else if (CheckSpaces(eForm.b_temp5_stime_m, "봉사활동시작시간 분")) {return false;}
		else if (CheckSpaces(eForm.b_temp5_etime_h, "봉사활동종료시간")) {return false;}
		else if (CheckSpaces(eForm.b_temp5_etime_m, "봉사활동종료시간 분")) {return false;}
		...
		*/
		...
		//봉사활동시간 최소시간 체크
		var b_temp5_stime_h = eval( eForm.b_temp5_stime_h.value );
		var b_temp5_stime_m = eval( eForm.b_temp5_stime_m.value );
		var b_temp5_etime_h = eval( eForm.b_temp5_etime_h.value );
		var b_temp5_etime_m = eval( eForm.b_temp5_etime_m.value );
		var time_chk = b_temp5_etime_h - b_temp5_stime_h;
		if(time_chk < 2){
			alert("최소 2시간 이상 신청하셔야 합니다.");
			eForm.b_temp5_stime_h.focus();
			return false;
		}else if(time_chk == 2){
			if(b_temp5_stime_m > b_temp5_etime_m){
				alert("최소 2시간 이상 신청하셔야 합니다.");
				eForm.b_temp5_stime_m.focus();
				return false;
			}
		}
		
		//희망시간대 범위 체크
		var b_temp2_sh = eval( eForm.b_temp2.value.substring(0,2) );
		var b_temp2_eh = eval( eForm.b_temp2.value.substring(6,8) );
		if(b_temp5_stime_h < b_temp2_sh || b_temp5_stime_h > b_temp2_eh){
			alert("봉사활동시간은 희망시간대를 벗어날 수 없습니다.");
			eForm.b_temp5_stime_h.focus();
			return false;
		}else if(b_temp5_etime_h < b_temp2_sh || b_temp5_etime_h > b_temp2_eh){
			alert("봉사활동시간은 희망시간대를 벗어날 수 없습니다.");
			eForm.b_temp5_etime_h.focus();
			return false;
		}
...
<div id="board" style="width:${config.a_width};">
	...
	<input type="hidden" name="b_temp6" value="${board.b_temp6 }" />
	</div>
	...
	<div class="table_bwrite">	
			...
			<tbody>
				...
				<tr>
				 ...
				 --%>
				 <tr>
					<th scope="row"><img alt="*(필수항목)" src="/nanum/site/board/nninc_calendar/img/ic_vcheck.gif"/> <label for="b_temp7">연락처</label></th>
					<td><input type="text" size="20" id="b_temp7" name="b_temp7" class="board_input" onfocus="focus_on1(this);" onblur="focus_off1(this);" value="${board.b_temp7}" maxlength="30" /></td>
				</tr>
				...
				<tr>
					<th scope="row"><label for="b_temp2"><img alt="*(필수항목)" src="/nanum/site/board/nninc_calendar/img/ic_vcheck.gif"/> 희망시간대</label></th>
					<td>
...
window.history.back();
				...
				<tr>
					<th scope="row"><label for="b_temp5"><img alt="*(필수항목)" src="/nanum/site/board/nninc_calendar/img/ic_vcheck.gif"/> 봉사활동시간</label></th>
					<td>
<c:if test="${!empty board.b_temp5 }">
	<c:set var="b_temp5_stime_h" value="${fn:substring(board.b_temp5,0,2) }" />
	<c:set var="b_temp5_stime_m" value="${fn:substring(board.b_temp5,3,5) }" />
	<c:set var="b_temp5_etime_h" value="${fn:substring(board.b_temp5,6,8) }" />
	<c:set var="b_temp5_etime_m" value="${fn:substring(board.b_temp5,9,11) }" />
</c:if>
						<input type="hidden" name="b_temp5" value="${board.b_temp5 }" />
						
						<div class="time_wrap">
						<span class="select_style medium">
						<select id="b_temp5_stime_h" name="b_temp5_stime_h">
							<option value="">시작시</option>
<c:forEach begin="9" end="16" var="i">
	<c:set var="i_value" value="${i }" />
	<c:if test="${i_value < 10 }"><c:set var="i_value" value="0${i_value }" /></c:if>
							<option value="${i_value }" ${b_temp5_stime_h eq i_value ? 'selected' : '' }>${i_value }</option>
</c:forEach>
						</select>
						</span>:
						<span class="select_style medium">
						<select id="b_temp5_stime_m" name="b_temp5_stime_m">
							<option value="">시작분</option>
							<option value="00" ${b_temp5_stime_m eq '00' ? 'selected' : '' }>00</option>
							<option value="30" ${b_temp5_stime_m eq '30' ? 'selected' : '' }>30</option>
						</select>
						</span>
						</div>
						~
						<div class="time_wrap">
						<span class="select_style medium">
						<select id="b_temp5_etime_h" name="b_temp5_etime_h">
							<option value="">시작시</option>
<c:set var="end_hour" value="17" />
<c:choose>
	<%-- 중앙도서관 평일 18시 처리 --%>
	<c:when test="${config.a_num eq '85409470'}">
		<c:if test="${thisweek ne '토' and thisweek ne '일'}">
			<c:set var="end_hour" value="18" />
		</c:if>
	</c:when>
	<%-- 금호평생 주말 16시 처리 --%>
	<c:when test="${config.a_num eq '41732388'}">
		<c:if test="${thisweek eq '토' or thisweek eq '일'}">
			<c:set var="end_hour" value="16" />
		</c:if>
	</c:when>
</c:choose>
<c:forEach begin="10" end="${end_hour}" var="i">
	<c:set var="i_value" value="${i }" />
	<c:if test="${i_value < 10 }"><c:set var="i_value" value="0${i_value }" /></c:if>
							<option value="${i_value }" ${b_temp5_etime_h eq i_value ? 'selected' : '' }>${i_value }</option>
</c:forEach>
						</select>
						</span>:
						<span class="select_style medium">
						<select id="b_temp5_etime_m" name="b_temp5_etime_m">
							<option value="">시작분</option>
							<option value="00" ${b_temp5_etime_m eq '00' ? 'selected' : '' }>00</option>
							<option value="30" ${b_temp5_etime_m eq '30' ? 'selected' : '' }>30</option>
						</select>
						</span>
						</div>
				...
				<tr>
					...
					<td>
						<input type="text" size="20" id="b_temp4" name="b_temp4" class="board_input" onfocus="focus_on1(this);" onblur="focus_off1(this);" value="${board.b_temp4}" maxlength="20" />
					</td>
				</tr>
<c:choose>
	...
	<div class="board_button2">
		<span><input id="submitbtn" class="con_btn own_btn" type="submit" value="저장" /></span>
