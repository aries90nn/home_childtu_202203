...
//<![CDATA[
	...
	function w_chk(){
		...
		else if (CheckSpaces(document.getElementById('frm').b_temp1, '유튜브 비디오 ID')) {return false;}
		else if ($("#youtubeChk").val() != "Y") {alert("유튜브 등록가능여부를 클릭해주세요."); return false;}
	...
	var youtubeKey = "<%=CommonConfig.get("youtubeKey")%>";
	var cannelId = "UCVP0Pc8muxJHTK-yi3MIs3Q";
	var dataPart = "snippet,id";
	var dataOrder = "date";

	function preView(id, thum){
		if(id == ""){
			alert("유튜브 비디오 ID를 입력해주세요.");
			$("#youtubeChk").focus();
			return false;
		}
		$.ajax({
			...
			dataType : "jsonp",
			cache : false,
			url : "https://www.googleapis.com/youtube/v3/videos?part=snippet,id&id="+id+"&key="+youtubeKey,
			success : function(data) {
				...
				$.each(data.items, function(key, value){
					$("#youtubeChk").val("Y");
					alert("등록이 가능합니다.");
					$(thum).html("<img src=\"https://img.youtube.com/vi/"+value.id+"/maxresdefault.jpg\" alt=\""+value.snippet.title+"\" style='width:350px' />");
					$("#b_temp2").val('https://img.youtube.com/vi/'+value.id+'/maxresdefault.jpg');
				});
			},
			error : function(data) {
				alert("다시 입력해주세요.");
...
<div id="board" style="width:${config.a_width};">
	...
	<input type="hidden" id="b_temp2" name="b_temp2" value="${board.b_temp2}" />
	<input type="hidden" id="youtubeChk" name="youtubeChk" value="${empty board.b_num ? 'N' : 'Y'}" />
	</div>
	...
	<div class="table_bwrite">
			...
			<tbody>
				...
				<tr class="btn_w2">
					<th scope="row"><label for="b_temp1"><img alt="*(필수항목)" src="/nanum/site/board/nninc_simple/img/ic_vcheck.gif"/> 유튜브 비디오 ID</label></th>
					<td><input type="text" id="b_temp1" name="b_temp1" class="board_input" onchange="checkReset();" onfocus="focus_on1(this);" onblur="focus_off1(this);" value="${board.b_temp1}" style="width:70%;" maxlength="100" />
						<input type="button" value="등록가능여부" onclick="javascript:preView($('#b_temp1').val(), $('#thum_img'));" class="NFU_cbtn NFU_cbtn_point">						
						<input type="button" value="ID 추출 방법" onclick="window.open('/main/site/popup_youtube.do','simple_pop','width=520,height=700,top=0,left=0,scrollbars=yes');" class="NFU_cbtn NFU_cbtn_g">
						<br/><span class="text1">*  유튜브 비디오 ID를 입력해주세요.</span>
					</td>
				</tr>
				
				<tr>
					<th scope="row"><label for="b_temp2">썸네일 미리보기</label></th>
					<td>
						<div id="thum_img">
							<c:if test="${!empty board.b_temp1}">
								<img src="https://img.youtube.com/vi/${board.b_temp1}/maxresdefault.jpg" style="width:350px" />
								<br/>
							</c:if>
							<span class="text1">*  썸네일이 존재하지 않거나 다른 메인 이미지를 희망하실 경우 아래 메인이미지파일에 이미지 파일을 첨부해주세요.</span>
						</div>
					</td>
				</tr>
