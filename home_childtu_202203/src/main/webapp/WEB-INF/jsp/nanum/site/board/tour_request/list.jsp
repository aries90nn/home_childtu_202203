...
<div id="board" style="width:${config.a_width};">
				...
				<c:forEach var="c" begin="0" end="6" step="1">
					...
					<td headers="schedule date${cday2}">
						...
						<c:forEach items="${calendarMap[thisDate]}" var="board" varStatus="no">
							...
							<a href="?proc_type=view&a_num=${config.a_num}&b_num=${board.b_num}&prepage=${nowPageEncode}" style="${board.b_temp8 eq '예약취소' ? 'text-decoration:line-through;' : ''}color:#6B6B6B;">${board.txt_show}
	...
	<div class="board_button">
		...
		※ 신청기관명을 클릭해서 신청상태를 확인할 수 있습니다.
		</p>
		<c:if test="${is_ad_cms eq 'Y'}">
		<p class="bot_info_red">
		※ 일반 사용자는 ${req_date}월(현재월+${req_month }) 이후부터는 신청불가로 보여집니다.
		</p>
		</c:if>
		</div>
		<div class="fr">
		<c:if test="${is_ad_cms eq 'Y'}">
			...
			<a href="/board/tour_request/tourExcel.do?a_num=${config.a_num}&sh_b_sdate_y=${thisYear[0]}&sh_b_sdate_m=${thisMonth[0]}" style="color:#006600;">엑셀자료받기</a>
			</p>
		</c:if>
		</div>
