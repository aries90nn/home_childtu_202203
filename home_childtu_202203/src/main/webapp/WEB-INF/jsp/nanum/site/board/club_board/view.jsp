...
<c:set var="list_url_tmp" value="${nowURI }?proc_type=list&a_num=${param.a_num }&v_cate=${board.b_cate }" />
...
<div id="board" style="width:${config.a_width};">
	...
	<div class="table_bview">
		...
		<thead>
			<tr>
				<th scope="col">${board.b_subject}</th>
			</tr>
		</thead>
		<tbody>
			<tr>
	...
	<div class="board_button">
		...
		<div class="fr pt10">
			...
			<c:if test="${is_write eq 'Y'}">
				<span><a href="?proc_type=write&a_num=${config.a_num}&b_num=${board.b_num}&v_cate=${param.v_cate }&prepage=${nowPageEncode}" class="cbtn cbtn_g">수정</a></span>
			</c:if>
			...
			<c:if test="${is_delete eq 'Y'}">
				<c:url var="gourl" value="/board/deleteOk.do">
					<c:param name="a_num" value="${config.a_num }" />
					<c:param name="b_num" value="${board.b_num }" />
					<c:param name="v_cate" value="${param.v_cate }" />
