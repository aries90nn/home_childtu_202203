...
<div id="board" style="width:${config.a_width};">
	...
	<c:choose>
		<c:when test="${fn:length(boardList) > 0}">
			...
			<li>
				<c:choose>
					<c:when test="${config.a_num eq '75543687' and board.b_num <= 1069 and board.b_num >= 762}"><c:set var="thum_yn"  value="N"/></c:when>
					<c:otherwise><c:set var="thum_yn"  value="Y"/></c:otherwise>
				</c:choose>
				<p class="img"><a href="?proc_type=view&a_num=${config.a_num}&b_num=${board.b_num}&prepage=${nowPageEncode}"><img src="/board/get_img.do?a_num=${config.a_num}&b_num=${board.b_num}&thum=${thum_yn }&f_num=1&type=img" class="photo" alt="${board.b_subject} 관련사진" /></a></p>
			...
			<!--  -->
			<c:if test="${status.index==(vCnt-1) or (i%4)==3}">
				<c:if test="${status.index==(vCnt-1) or (i%4)!=3}">
					<li colspan='${3-(i%4)}'>&nbsp;</li>
				</c:if>
			</c:if>
