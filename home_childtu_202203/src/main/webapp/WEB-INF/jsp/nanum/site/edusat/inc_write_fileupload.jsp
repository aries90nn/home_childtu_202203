...
<c:forEach var="t" begin="1" end="5" step="1">
	...
	<c:set var="es_file_name">${edurequest[file_name]}</c:set>
	<input type="hidden" id="es_file${t}" name="es_file${t}" value="${es_file_name}" />
	
	<c:if test="${es_file_name ne ''}">
		${es_file_name} <a href="#delete" onclick="delete_ok('${es_idx}','${edu_idx}','${t}','${del_rtn_page}');" onkeyup="delete_ok('${es_idx}','${edu_idx}','${t}','${del_rtn_page}');" title="첨부파일 삭제하기">
...
<div id="NFU_file_control" style="width:98%;height:120px;"></div>
	...
	<div>
		<span style="font-size:12px;color:#DF0101">※ 허용파일가능갯수 :${5-file_count}개 <br/>※ 제한용량 : 5MB <br/>※ 허용파일은 gif,jpg,png,jpeg,bmp </span>
	</div>
