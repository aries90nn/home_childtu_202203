...
<script type="text/javascript">//<![CDATA[
	...
	function w_chk(){
		...
		if (CheckSpaces(eForm.b_subject, '신청기관명')) {return false;}
		else if (CheckSpaces(eForm.b_temp1_1, "신청기관 연락처 첫번째")) {return false;}
		else if (CheckSpaces(eForm.b_temp1_2, "신청기관 연락처 두번째")) {return false;}
		else if (CheckSpaces(eForm.b_temp1_3, "신청기관 연락처 세번째")) {return false;}
		...
		else if (CheckSpaces(document.getElementById('frm').b_name, '신청자 성명')) {return false;}
		...
		<c:if test="${config.a_num eq '22834449'}">
		else if (CheckSpaces(document.getElementById('frm').b_temp7, '예상방문시간')) {return false;}
		</c:if>
		else if (CheckSpaces(eForm.b_temp4, "방문인원")) {return false;}

		else if (CheckSpaces(eForm.b_temp5, "인솔자 성명")) {return false;}
		else if (CheckSpaces(eForm.b_temp6_1, "인솔자 연락처 첫번째")) {return false;}
		else if (CheckSpaces(eForm.b_temp6_2, "인솔자 연락처 두번째")) {return false;}
		else if (CheckSpaces(eForm.b_temp6_3, "인솔자 연락처 세번째")) {return false;}
	...
	<div class="table_bwrite">	
			...
			<thead>
				<tr>
					...
					${fn:substring(board.b_sdate, 0, 4)}년 ${fn:substring(board.b_sdate, 5, 7)}월 ${fn:substring(board.b_sdate, 8,10)}일 견학신청서 작성</th>
				</tr>
			</thead>
			<tbody>
				<tr>
					<th scope="row" rowspan="${config.a_home eq 'Y' ? '3' : '2'}"><label for="b_subject">신청기관</label></th>
					<th scope="row"><label for="b_subject"><img alt="*(필수항목)" src="/nanum/site/board/nninc_simple/img/ic_vcheck.gif"/> 기관명</label></th>
					<td><input type="text" size="90" id="b_subject" name="b_subject" class="board_input" onfocus="focus_on1(this);" onblur="focus_off1(this);" value="${board.b_subject}" maxlength="200" /></td>
				</tr>

				<tr>
					<th scope="row"><img alt="*(필수항목)" src="/nanum/site/board/nninc_calendar/img/ic_vcheck.gif"/> <label for="b_temp1_1">연락처</label></th>
					<td>
					<c:set var="b_temp1_arr" value="${fn:split(board.b_temp1,'-')}" />
					<input type="text" size="4" title="연락처 첫번째" id="b_temp1_1" name="b_temp1_1" class="board_input" onfocus="focus_on1(this);" onblur="SetNum(this);focus_off1(this);" value="${b_temp1_arr[0]}" maxlength="4" /> - <input type="text" size="4" title="연락처 두번째" id="b_temp1_2" name="b_temp1_2" class="board_input" onfocus="focus_on1(this);" onblur="SetNum(this);focus_off1(this);" value="${b_temp1_arr[1]}" maxlength="4" /> - <input type="text" size="4" title="연락처 세번째" id="b_temp1_3" name="b_temp1_3" class="board_input" onfocus="focus_on1(this);" onblur="SetNum(this);focus_off1(this);" value="${b_temp1_arr[2]}" maxlength="4" /></td>
				</tr>
				...
				<tr>
					<th scope="row" rowspan="3"><label for="b_name">신청자</label></th>
				...
				<tr>
					<th scope="row"><img alt="*(필수항목)" src="/nanum/site/board/nninc_calendar/img/ic_vcheck.gif"/> <label for="b_email">이메일</label></th>
				...
				<c:if test="${board.b_pwd eq ''}">
				<tr>
					<th scope="row" colspan="2"><label for="b_pwd"><img alt="*(필수항목)" src="/nanum/site/board/nninc_calendar/img/ic_vcheck.gif"/> 비밀번호</label></th>
				...
				<tr>
					<th scope="row" colspan="2"><label for="b_temp2"><img alt="*(필수항목)" src="/nanum/site/board/nninc_calendar/img/ic_vcheck.gif"/> 방문시간대</label></th>
					<td>
...
window.history.back();
				...
				<tr>
					<th scope="row" colspan="2"><label for="b_temp3">방문자 연령대</label></th>
				...
				<tr>
					<th scope="row" colspan="2"><img alt="*(필수항목)" src="/nanum/site/board/nninc_calendar/img/ic_vcheck.gif"/> <label for="b_temp4">방문인원</label></th>
					<td style="padding-top:5px;padding-bottom:5px;">
						<c:set var="max_inwon" value="30" />
						<c:if test="${config.a_num eq '35777134'}">
							<c:set var="max_inwon" value="25" />
						</c:if>
						<span class="select_style">
						<select id="b_temp4" name="b_temp4">
							<option value="">인원선택</option>
							<c:forEach var="i" begin="1" end="${max_inwon}" step="1">
							<option value="${i}" ${board.b_temp4 eq i ? "selected" : ""}>${i}명</option>
							</c:forEach>
						</select>
						</span>
						<br /><span class="text1">* 방문 인원은 ${max_inwon}명 이하만 신청 가능합니다.
						<!-- <br />30명을 초과할 경우 담당자와 사전협의가 필요합니다.  [연락처 : 053)663-3701]</span> -->
					</td>
				</tr>

				<tr>
					<th scope="row" rowspan="2"><label for="b_temp5">인솔자</label></th>
					<th scope="row"><label for="b_temp5"><img alt="*(필수항목)" src="/nanum/site/board/nninc_calendar/img/ic_vcheck.gif"/> 성명</label></th>
					<td><input type="text" size="20" id="b_temp5" name="b_temp5" class="board_input" onfocus="focus_on1(this);" onblur="focus_off1(this);" value="${board.b_temp5}" maxlength="30" /></td>
				</tr>

				<tr>
					<th scope="row"><label for="b_temp6"><img alt="*(필수항목)" src="/nanum/site/board/nninc_calendar/img/ic_vcheck.gif"/> 연락처</label></th>
					<td>
					<c:set var="b_temp6_arr" value="${fn:split(board.b_temp6,'-')}" />
					<input type="text" size="4" title="연락처 첫번째" id="b_temp6_1" name="b_temp6_1" class="board_input" onfocus="focus_on1(this);" onblur="SetNum(this);focus_off1(this);" value="${b_temp6_arr[0]}" maxlength="4" /> - <input type="text" size="4" title="연락처 두번째" id="b_temp6_2" name="b_temp6_2" class="board_input" onfocus="focus_on1(this);" onblur="SetNum(this);focus_off1(this);" value="${b_temp6_arr[1]}" maxlength="4" /> - <input type="text" size="4" title="연락처 세번째" id="b_temp6_3" name="b_temp6_3" class="board_input" onfocus="focus_on1(this);" onblur="SetNum(this);focus_off1(this);" value="${b_temp6_arr[2]}" maxlength="4" /></td>
				</tr>
<c:choose>
	...
	<c:otherwise>
				...
				<tr>
					<th scope="row" colspan="2"><img alt="*(필수항목)" src="/nanum/site/board/nninc_simple/img/ic_vcheck.gif"/> <label for="b_cate_tot">분류</label></th>
					<td>
						<span class="select_style">
				...
				<tr>
					<th colspan="2" scope="row">첨부파일</th>
