...
<c:set var="a_nofilesize" value="100" />
<c:set var="ct_menu_folder" value="${ct_menu_folder}" />
...
function fileUploadResultEvent(result){
	...
	for(var i=1;i<=nfu_upload_count;i++){
		ebp_pageimg =  document.getElementById("img_"+i);
...
※ gif,jpg,png파일만 가능합니다.<br />

<c:forEach var="i" begin="1" end="${a_upload_len}" step="1">
	<input type="hidden" id="img_${i}" name="img_data" value="" />
</c:forEach>
...
NFileUpload.uploadURL = "./nfu_upload.do?ct_menu_folder=${ct_menu_folder}";			//파일업로드 처리URL
NFileUploadNormal.uploadURL = "./nfu_normal_upload.do?ct_menu_folder=${ct_menu_folder}";	//일반파일업로드 처리URL
