	...
	$("#sidemenu a.category").each(function(){
		...
		if (obj.length>0){
			if($(this).parent("li").data("chk") == "Y"){
				$(this).children("img.group").attr("src",cmsFolderIcon[5]);
			}else{
				$(this).children("img.group").attr("src",cmsFolderIcon[3]);
			}
		}else{
			if($(this).parent("li").data("chk") == "Y"){
...
// --- 이벤트설정 ---- //
	...
	$("#sidemenu a.category").rightClick( function(e) {
		fn_menuInfoView($(this));
	});
	...
	$("#sidemenu a.category").click(function(){
		...
		if (obj.length>0){	// 하위 메뉴있으면
			if (obj.filter(":hidden").length>0){	// 가려져있으면
				obj.show();
				if($(this).parent("li").data("chk") == "Y"){
					$(this).children("img.group").attr("src",cmsFolderIcon[5]);
				}else{
	...
	// 메뉴명(텍스트) 더블클릭 -> 내용설정
	$("#sidemenu a.category").dblclick(function(){
		nowMenuIdx = $(this).parent("li").data("idx");
		cms_menu2();
	});
