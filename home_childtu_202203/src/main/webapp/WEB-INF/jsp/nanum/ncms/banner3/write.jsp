...
<c:set var="prepage" value="${empty param.prepage ? '/ncms/banner3/list.do' : param.prepage }" />
<c:if test="${empty param.prepage and !empty BUILDER_DIR }">
	<c:set var="prepage" value="/${BUILDER_DIR }${prepage }" />
</c:if>
<script type="text/javascript" src="/nanum/ncms/common/js/ncms_banner3.js"></script>

<h1 class="tit"><span>${banner3.b_l_num == null or banner3.b_l_num eq '' ? "신규 팝업 생성" : "팝업 수정"}</span></h1>
<!-- 내용들어가는곳 -->
<div id="contents_area">
	...
	<div>
	<input type="hidden" name="b_l_num" value="${banner3.b_l_num}" /><!-- (수정일때사용) -->
	<input type="hidden" name="b_l_img2" value="${banner3.b_l_img}" />
	<input type="hidden" name="prepage" value="${prepage}" />
		...
		<fieldset>
			...
			<tr>
				...
				<td class="left">
					<input type="text" title="제목 입력" id="b_l_subject" name="b_l_subject" class="ta_input w5" value="${banner3.b_l_subject}" maxlength="50" />
			...
			<tr>
				...
				<jsp:useBean id="date" class="java.util.Date" />
					...
					&nbsp;&nbsp;
					<input type="checkbox" name="unlimited" id="unlimited" value="Y"  onclick="javascript: click_limit();" <c:if test='${banner3.unlimited == "Y"}'> checked</c:if> /><label for="unlimited">무제한</label>
			...
			<tr>
				...
				<td class="left">
					...
					<c:if test="${!empty banner3.b_l_img}">
						<br/><img src = "/data/banner3/${banner3.b_l_img}" alt="${banner3.b_l_subject}" class="vam pt5" />
						&nbsp;<input type='checkbox' name='b_l_img_del' value='${banner3.b_l_img}' />삭제
					</c:if>
			...
			<tr>
				...
				<td class="left"><input type="text" size="80" title="연결주소 입력" id="b_l_url" name="b_l_url" class="ta_input" value="${banner3.b_l_url}" maxlength="300" />
				<input type="checkbox" name="b_l_win" id="b_l_win" value="1" ${banner3.b_l_win eq '1' ? 'checked="checked"' : '' } /><label for="b_l_win">새창</label>
			...
			<tr>
				...
				<td class="left">
					<select id="b_l_chk" name="b_l_chk" title="팝업 사용여부 선택" class="ta_select" style="width:60px;" >
						<option value="Y"  ${banner3.b_l_chk eq 'Y' ? 'selected="selected"' : '' }>사용</option>
						<option value="N" ${banner3.b_l_chk eq 'N' or banner3.b_l_chk eq '' ? 'selected="selected"' : '' }>중지</option>
					</select>
				</td>
			</tr>
			</table>
