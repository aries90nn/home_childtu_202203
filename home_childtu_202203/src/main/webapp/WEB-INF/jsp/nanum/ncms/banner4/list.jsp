...
function deleteOk(b_l_num){
	...
	<div id="contents_area">
			...
			<div class="top_search_area mt10">
				<ul>
					<li class="tit"><label for="v_search"><h3 class="tit">통합팝업검색 :</h3></label></li>
					<li class="sel">
		...
		<form id= "frm_list" action="" method='post'>
			...
			<fieldset>
				<legend>통합팝업목록보기</legend>
				<table class="bbs_common bbs_default" summary="사이트의 통합팝업를 관리합니다.">
				<caption>통합팝업관리 서식</caption>
				<colgroup>
				<col width="50" />
				<col />
				<col width="20%" />
				...
				<tr>
					<td class="center"><input type="checkbox" name="chk" value="${banner4.b_l_num}" title="해당 통합팝업 선택" /></td>
					<td class="center p0">
						<a href="javascript:page_go2('${banner4.b_l_url}');" >
						<img src = "/data/banner4/${banner4.b_l_img}" alt="${banner4.b_l_subject}" style="max-width:200px;" /></a>
					</td>
					<td class="center eng"><span class="point">${fn:substring(banner4.b_l_sdate,0,10)} ~ 
						<c:choose>
							<c:when test="${banner4.unlimited eq 'Y'}">
								무제한
							</c:when>
							<c:otherwise>
								${fn:substring(banner4.b_l_edate,0,10)}
							</c:otherwise>
						</c:choose>
					</span></td>
					<td class="center">${banner4.b_l_chk == "Y" ? "<strong>사용</strong>" : "중지"}</td>
					<td class="center"><a href="write.do?b_l_num=${banner4.b_l_num}&prepage=${nowPageEncode}"><img alt="수정" src="/nanum/ncms/img/common/modify_icon.gif"/></a></td>
					<td class="center"><a href="#del" onclick="deleteOk('${banner4.b_l_num}');"><img alt="삭제" src="/nanum/ncms/img/common/delete_icon.gif" /></a></td>
				</tr>
				</c:forEach>
			...
			<div id="contoll_area">
				<ul>
					<li class="btn_le"><p><a href="javascript:checkAll();" class="btn_bl">전체 선택/해제</a></p><p><a href="javascript:delete2();" class="btn_gr">선택 통합팝업삭제</a></p>
