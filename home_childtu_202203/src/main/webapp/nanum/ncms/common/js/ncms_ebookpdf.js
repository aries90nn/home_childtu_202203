function w_chk(){
	if (CheckSpaces(frm.eb_subject, '제목')) { return false; }
	if ($("#eb_pdf_org").length == 0){
		if (CheckSpaces(frm.eb_pdf_file, '소식지 pdf파일')) { return false; }
	}
}
...
function deleteOk(idx){
	var prepage_val = "";
	try {
		prepage_val = document.getElementById("frm_list").prepage.value;
	}catch(e){}
	
	if(confirm("삭제하시겠습니까?")){
		location.href="deleteOk.do?idx="+idx+"&prepage="+escape(prepage_val);
	}
}
