...
function w_chk() {
	var frm = document.getElementById('form_write');
	
	if(CheckSpaces(frm.st_date, "날짜")) {return false;}
	else if(CheckSpaces(frm.st_work, "작업")) {return false;}
	else if(CheckSpaces(frm.st_cnt, "수량")) {return false;}
	else if(frm.st_work.value != "입고") {
		if(CheckSpaces(frm.st_program, "프로그램")) {return false;}
		else if(CheckSpaces(frm.st_delivery, "배송정보")) {return false;}	
	}
	loading_st(1);
}
