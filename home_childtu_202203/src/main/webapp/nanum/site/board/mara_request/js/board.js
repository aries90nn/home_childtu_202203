...
function typeChange(){
	$("#b_temp2,#b_temp3_1,#b_temp3_2").attr("disabled", false).css("background-color", "");
	if($("#b_temp1").val() == "4"){
		$("#b_temp2,#b_temp3_1,#b_temp3_2").val("").attr("disabled", true).css("background-color", "#EEEEEE");
	}
}

function courseChange(){
	$("#b_temp7>option").attr("disabled", false);
	
	switch($("#b_temp1").val()){
		case "1" :		//초저
			if($("#b_temp7").val() == "5000" || $("#b_temp7").val() == "10000"){
				alert("초등학교저학년은 3km, 하프코스만 선택이 가능합니다.");
				$("#b_temp7").val("");
			}
			$("#b_temp7>option[value='5000'],#b_temp7>option[value='10000']").attr("disabled", true);
			break;
		case "5" :		//초고
			if($("#b_temp7").val() == "3000" || $("#b_temp7").val() == "10000"){
				alert("초등학교고학년은 5km, 하프코스만 선택이 가능합니다.");
				$("#b_temp7").val("");
			}
			$("#b_temp7>option[value='3000'],#b_temp7>option[value='10000']").attr("disabled", true);
			break;
		case "2" : case "3" : case "4" :		//중,고,일반
			if($("#b_temp7").val() == "3000" || $("#b_temp7").val() == "5000"){
				alert("중고등학생 및 일반인은 10km, 하프코스만 선택이 가능합니다.");
				$("#b_temp7").val("");
			}
			$("#b_temp7>option[value='3000'],#b_temp7>option[value='5000']").attr("disabled", true);
			break;
	}
}
...
function statusChange(){
	...
	if (confirm('참가상태를 변경하시겠습니까?'))	{
		...
		if (flag == "Empty")	{
			...
			loading_st(1);
			document.getElementById('frm_list').action = "/board/mara_request/statusOk.do";
			document.getElementById('frm_list').status.value = "totdel";
			document.getElementById('frm_list').submit();
