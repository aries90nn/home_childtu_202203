...
function w_chk(){
	...
	//else if (CheckSpaces(document.getElementById('frm').b_l_img_file, '배너 이미지파일')) { return false; }
...
//삭제
function d_chk(b_l_num){
	
	if (confirm('정말 삭제하시겠습니까?'))	{
		
		document.getElementById('frm_list').action = "deleteOk.do";
		document.getElementById('frm_list').b_l_num.value=b_l_num;
...
function delete2(page){
	...
	if (confirm('정말 삭제하시겠습니까?'))	{
		...
		if (flag == "Empty")	{
			...
			loading_st(1);
			document.getElementById('frm_list').action = "/ncms/banner/deleteOk.do?"+page;
...
function tot_levelchage(page){
	...
	if (document.getElementById('frm_list').tot_level_chk.value=="")	{
		...
		loading_st(1);
		document.getElementById('frm_list').action = "/ncms/banner/levelOk.do?"+page;
