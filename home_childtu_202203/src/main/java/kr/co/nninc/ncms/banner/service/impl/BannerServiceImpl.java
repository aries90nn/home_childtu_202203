package kr.co.nninc.ncms.banner.service.impl;
...
public class BannerServiceImpl  extends EgovAbstractServiceImpl implements BannerService{
	...
	public void list(Model model) throws Exception {
		...
		selectdto.setExColumn(new BannerDTO());
		...
		List<HashMap<String,String>> bannerList = exdao.selectQueryTable(selectdto);
		// setAttribute
	...
	public void modify(Model model) throws Exception {
		...
		selectdto.setExColumn("b_l_num, info_url, b_l_code, b_l_img, b_l_url, b_l_chk, b_l_sdate, b_l_edate, b_l_subject, b_l_memo, b_l_wdate, b_l_mdate, unlimited");
		...
		String b_l_sdate = Func.nvl(banner.get("b_l_sdate"));
		if(!"".equals(b_l_sdate)) {
		...
		String b_l_edate = Func.nvl(banner.get("b_l_edate"));
		if(!"".equals(b_l_edate)) {
		...
		//작업기록
		Func.writeManagerLog ("배너 상세보기 ["+banner.get("b_l_subject")+"]", request);
	...
	public String writeOk(Model model) throws Exception {
		...
		BannerDTO bannerdto = (BannerDTO) map.get("bannerdto");
		...
		if("".equals(Func.nvl( request.getParameter("unlimited") ))) bannerdto.setUnlimited("N");
		...
		// 파일 유효성 검사 및 저장
		FileDTO file = fileutil.wfile(model, multi.getFileMap(), uploaddir, true, "배너이미지", "b_l_img_file", 5 * 1024 * 1024, fileutil.FILE_TYPE_IMAGE);
		if(!"".equals(Func.nvl(file.getFile_name()))) {
			bannerdto.setB_l_img(Func.nvl(file.getFile_name()));
		}else if(!"".equals(Func.nvl(file.getError_msg()))){
			return messageService.backMsg(model, file.getError_msg());
		}
		
		//순서업데이트
		exdao.executeQuery("UPDATE banner SET B_L_CODE = B_L_CODE + 1");
		
		// 세팅
		bannerdto.setB_l_code("1");
		bannerdto.setB_l_sdate(b_l_sdate);
		bannerdto.setB_l_edate(b_l_edate);
		bannerdto.setB_l_wdate(b_l_wdate);
		bannerdto.setB_l_mdate(b_l_mdate);
		
		//insert 제외 필드
		bannerdto.setB_l_num(null);
		
		//site 디렉토리 저장
		String builder_dir = Func.nvl( (String)request.getAttribute("BUILDER_DIR") ).trim();
		if(!"".equals(builder_dir)){
			bannerdto.setSite_dir(builder_dir);
		}
		
		//insert
		bannerdto.setExTableName("banner");
		exdao.insert(bannerdto);
		
		//작업기록
		Func.writeManagerLog ("배너 생성 ["+bannerdto.getB_l_subject()+"]", request);
	...
	public String modifyOk(Model model) throws Exception {
		...
		if("".equals(unlimited)) bannerdto.setUnlimited("N");
		...
		// 파일 유효성 검사 및 저장
		bannerdto.setB_l_img("");
		FileDTO file = fileutil.wfile(model, multi.getFileMap(), uploaddir, img_chk, "배너이미지", "b_l_img_file", 5 * 1024 * 1024, fileutil.FILE_TYPE_IMAGE);
		if(!"".equals(Func.nvl(file.getFile_name()))) {
			bannerdto.setB_l_img(Func.nvl(file.getFile_name()));
		}else if(!"".equals(Func.nvl(file.getError_msg()))){
			return messageService.backMsg(model, file.getError_msg());
		...
		if(!"".equals(bannerdto.getB_l_img())){  //첨부파일이 있으면
			Func.fileDelete(uploaddir+"/"+b_l_img2);
		}else{
			bannerdto.setB_l_img(b_l_img2);
		...
		//수정제외 필드
		bannerdto.setB_l_num(null);
		bannerdto.setSite_dir(null);
		
		
		//수정
		bannerdto.setExTableName("banner");
		bannerdto.setExWhereQuery("where b_l_num = '"+b_l_num+"'");
		exdao.update(bannerdto);
		
		//작업기록
		Func.writeManagerLog ("배너 수정 ["+bannerdto.getB_l_subject()+"]", request);
	...
	public void deleteOk(Model model) throws Exception {
		...
		if (status.equals("totdel")) { //다중삭제
			for(int z=0;z <= chk.length-1;z++){
				...
				HashMap<String,String>banner = exdao.selectQueryRecord(selectdto);
				Func.fileDelete(uploaddir+"/"+banner.get("b_l_img"));
				exdao.executeQuery("delete from banner where b_l_num = '"+chk[z]+"'");

				//작업기록
				Func.writeManagerLog ("배너 삭제 ["+banner.get("b_l_subject")+"]", request);
			}
		} else { //단일삭제
			selectdto.setExWhereQuery("where b_l_num = '"+b_l_num+"'");
			HashMap<String,String>banner = exdao.selectQueryRecord(selectdto);
			...
			//파일삭제
			Func.fileDelete(uploaddir+"/"+banner.get("b_l_img"));
			exdao.executeQuery("delete from banner where b_l_num = '"+b_l_num+"'");
	...
	public void listMoveOk(Model model) throws Exception {
		...
		for (int z = 0; z <= chk.length - 1; z++) {
			...
			exdao.executeQuery("UPDATE banner SET B_L_CODE = '"+Func.cStr(z + 1)+"' WHERE B_L_NUM = '"+b_l_num+"'");
		}
		//작업기록
		Func.writeManagerLog ("배너 일괄순서변경", request);
	}
	...
	public void levelOk(Model model) throws Exception {
		...
		for (int z = 0; z <= chk.length - 1; z++) {
			...
			exdao.executeQuery("UPDATE banner SET B_L_CHK = '"+b_l_chk+"' WHERE B_L_NUM = '"+b_l_num+"'");
			...
			Func.writeManagerLog ("배너 상태변경 ["+b_l_subject+"("+b_l_chk_str+")]", request);
