package kr.co.nninc.ncms.banner3.service.impl;
 ...
 * @version 1.0
 */
@Service("banner3Service")
public class Banner3ServiceImpl extends EgovAbstractServiceImpl implements Banner3Service {
	...
	public void modify(Model model) throws Exception {
		...
		HashMap<String,String>banner3 = exdao.selectQueryRecord(selectdto);
		...
		String b_l_sdate = Func.nvl(banner3.get("b_l_sdate"));
		if(!"".equals(b_l_sdate)) {
		...
		String b_l_edate = Func.nvl(banner3.get("b_l_edate"));
		if(!"".equals(b_l_edate)) {
		...
		//작업기록
		Func.writeManagerLog ("상단팝업 상세보기 ["+banner3.get("b_l_subject")+"]", request);
	...
	public String writeOk(Model model) throws Exception {
		...
		Banner3DTO banner3dto = (Banner3DTO) map.get("banner3dto");
		...
		if("".equals(Func.nvl( request.getParameter("unlimited") ))) banner3dto.setUnlimited("N");
		
		//======= 저장공간인 [DATA]폴더가 없다면 생성 한다. ================
		String uploaddir =  fileutil.realPath(request, "/data/banner3/");
		...
		if(!"".equals(Func.nvl(file.getFile_name()))) {
			banner3dto.setB_l_img(Func.nvl(file.getFile_name()));
		}else if(!"".equals(Func.nvl(file.getError_msg()))){
			return messageService.backMsg(model, file.getError_msg());
		}
		
		//순서업데이트
		exdao.executeQuery("UPDATE banner3 SET B_L_CODE = B_L_CODE + 1");
		
		// 세팅
		banner3dto.setB_l_code("1");
		banner3dto.setB_l_sdate(b_l_sdate);
		banner3dto.setB_l_edate(b_l_edate);
		banner3dto.setB_l_wdate(b_l_wdate);
		banner3dto.setB_l_mdate(b_l_mdate);
		
		//insert 제외 필드
		banner3dto.setB_l_num(null);
		
		//site 디렉토리 저장
		String builder_dir = Func.nvl( (String)request.getAttribute("BUILDER_DIR") ).trim();
		if(!"".equals(builder_dir)){
			banner3dto.setSite_dir(builder_dir);
		}
		
		//insert
		banner3dto.setExTableName("banner3");
		exdao.insert(banner3dto);
		
		//작업기록
		Func.writeManagerLog ("상단팝업 생성 ["+banner3dto.getB_l_subject()+"]", request);
	...
	public String modifyOk(Model model) throws Exception {
		...
		if("".equals(unlimited)) banner3dto.setUnlimited("N");
		...
		if(!"".equals(banner3dto.getB_l_img())){  //첨부파일이 있으면
			Func.fileDelete(uploaddir+"/"+b_l_img2);
		}else{
			banner3dto.setB_l_img(b_l_img2);
		...
		banner3dto.setB_l_edate(b_l_edate);
		banner3dto.setB_l_mdate(b_l_mdate);
		if(banner3dto.getB_l_win() == null){banner3dto.setB_l_win("");}
		
		//수정제외 필드
		banner3dto.setB_l_num(null);
		banner3dto.setSite_dir(null);
		
		
		//수정
		banner3dto.setExTableName("banner3");
		banner3dto.setExWhereQuery("where b_l_num = '"+b_l_num+"'");
		exdao.update(banner3dto);
		
		//작업기록
		Func.writeManagerLog ("상단팝업 수정 ["+banner3dto.getB_l_subject()+"]", request);
	...
	public void deleteOk(Model model) throws Exception {
		...
		if (status.equals("totdel")) { //다중삭제
			for(int z=0;z <= chk.length-1;z++){
				...
				HashMap<String,String>banner3 = exdao.selectQueryRecord(selectdto);
				Func.fileDelete(uploaddir+"/"+banner3.get("b_l_img"));
				exdao.executeQuery("delete from banner3 where b_l_num = '"+chk[z]+"'");

				//작업기록
				Func.writeManagerLog ("상단팝업 삭제 ["+banner3.get("b_l_subject")+"]", request);
			}
		} else { //단일삭제
			selectdto.setExWhereQuery("where b_l_num = '"+b_l_num+"'");
			HashMap<String,String>banner3 = exdao.selectQueryRecord(selectdto);
			...
			//파일삭제
			Func.fileDelete(uploaddir+"/"+banner3.get("b_l_img"));
			exdao.executeQuery("delete from banner3 where b_l_num = '"+b_l_num+"'");
	...
	public void listMoveOk(Model model) throws Exception {
		...
		for (int z = 0; z <= chk.length - 1; z++) {
			...
			exdao.executeQuery("UPDATE banner3 SET B_L_CODE = '"+Func.cStr(z + 1)+"' WHERE B_L_NUM = '"+b_l_num+"'");
		}
		//작업기록
	...
	public void levelOk(Model model) throws Exception {
		...
		for (int z = 0; z <= chk.length - 1; z++) {
			...
			exdao.executeQuery("UPDATE banner3 SET B_L_CHK = '"+b_l_chk+"' WHERE B_L_NUM = '"+b_l_num+"'");
